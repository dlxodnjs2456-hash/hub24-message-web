...
export default function ReferralPage(){
 ...
 return <main className="content" style={{maxWidth:1250,margin:'0 auto',paddingTop:34}}><div className="head"><div><span className="eyebrow">N PAY REFERRAL</span><h1 style={{margin:'7px 0 5px'}}>추천인</h1><p>추천 회원의 정상적인 충전·출금 이용에 따라 1% 추천 보상이 지급됩니다.</p></div><button className="btn" onClick={load}>새로고침</button></div>
 ...
 <section className="card" style={{marginTop:14}}><div className="head"><div><h2>{board.month} 추천왕</h2><p>첫 승인 충전 기준을 충족한 유효 추천회원만 집계합니다. 매월 1일 새 집계가 자동 시작됩니다.</p></div>{board.king&&<span className="badge ok">현재 1위 · {board.king.name}</span>}</div><div className="table"><table><thead><tr><th>순위</th><th>회원</th><th>유효 추천</th><th>이번달 추천보상</th></tr></thead><tbody>{board.items?.length?board.items.map(r=><tr key={r.user_id} style={r.is_me?{fontWeight:900}:null}><td>{r.rank===1?'👑 1':r.rank}</td><td>{r.name}{r.is_me?' (나)':''}</td><td>{Number(r.referral_count||0).toLocaleString()}명</td><td>{fmt(r.reward_amount)}</td></tr>):<tr><td colSpan="4" className="empty">이번달 유효 추천 실적이 없습니다.</td></tr>}</tbody></table></div></section>
 <section className="card" style={{marginTop:14}}><h2>추천 보상 보호 기준</h2><div className="note">유효 추천 인정 기준: 첫 승인 충전 {fmt(me?.qualification_charge||10000)} 이상. 충전 승인 및 출금 지급완료 금액의 1%가 보상되며 동일 건은 중복 지급되지 않습니다. 반복 자금순환 악용 방지를 위해 추천회원 1명당 월 보상 상한은 {fmt(me?.monthly_reward_cap_per_referred||100000)}입니다.</div></section></main>;
}
