...
export default function MessagesPage(){
 ...
 return <main className="marketChatPage">
  <div className="sectionTitle"><div><span className="eyebrow">N PAY MESSAGE</span><h1>1:1 대화</h1><p>구매 전 판매자와 사이트 내부에서 대화할 수 있습니다.</p></div><a className="softButton" href="/market">자유시장으로</a></div>
  <section className="marketChatWorkspace">
   <aside className="marketChatThreads">{threads.length?threads.map(t=><button key={t.id} className={selected?.id===t.id?'active':''} onClick={()=>openThread(t)}><b>{t.product?.title||t.seller?.seller_name||'1:1 대화'}</b><span>{t.seller?.seller_name||'판매자'} {t.seller?.telegram_username?`· ${t.seller.telegram_username}`:''}</span><small>{dt(t.updated_at)}</small></button>):<div className="marketChatEmpty">대화가 없습니다.</div>}</aside>
   <div className="marketChatRoom">{selected?<><div className="marketChatHead"><div><b>{threads.find(x=>x.id===selected.id)?.product?.title||'상품 문의'}</b><span>{threads.find(x=>x.id===selected.id)?.seller?.seller_name||'판매자'}</span></div></div><div className="marketChatMessages">{messages.length?messages.map(m=><div key={m.id} className={'marketChatBubble '+(String(m.sender_id)===String(session.user.id)?'mine':'other')}><p>{m.message}</p><small>{dt(m.created_at)}</small></div>):<div className="marketChatEmpty">첫 메시지를 보내보세요.</div>}</div><div className="marketChatComposer"><input value={text} onChange={e=>setText(e.target.value)} onKeyDown={e=>e.key==='Enter'&&send()} placeholder="메시지를 입력하세요." maxLength={4000}/><button onClick={send}>전송</button></div></>:<div className="marketChatEmpty">왼쪽에서 대화를 선택하세요.</div>}</div>
  </section>
 </main>;
}
