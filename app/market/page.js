...
export default function MarketPage(){
 ...
 async function load(category=''){
   try{
     const [c,p,s,t]=await Promise.all([api.marketCategories(),api.marketProducts(category),api.marketSellers(),api.marketTrades()]);
 ...
 return <main className="marketPage">
   ...
   <div className="marketViewTabs"><button className={view==='products'?'active':''} onClick={()=>setView('products')}>전체 상품</button><button className={view==='sellers'?'active':''} onClick={()=>setView('sellers')}>직접 거래 판매자</button><button className={view==='trades'?'active':''} onClick={()=>setView('trades')}>내 거래</button></div>

   {view==='products'&&<>
    <section className="shopToolbar"><div className="marketSearch"><span>⌕</span><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="상품명 또는 판매자 검색"/></div><select value={sort} onChange={e=>setSort(e.target.value)}><option value="latest">최신순</option><option value="low">가격 낮은순</option><option value="high">가격 높은순</option><option value="sales">판매 많은순</option></select></section>
    ...
    <section className="productGrid">{shownProducts.length?shownProducts.map(p=><article className="shopCard" key={p.id}><div className="shopCardTop"><span>{p.category?.name||'기타'}</span><small>{p.stock==null?'재고 제한 없음':`재고 ${p.stock}`}</small></div><div className="productVisual"><span>{String(p.title||'H').slice(0,1).toUpperCase()}</span></div><div className="shopCardBody"><h3>{p.title}</h3><p>{p.description||'상품 설명이 없습니다.'}</p><div className="sellerStrip"><div className="sellerAvatar">{String(p.seller?.seller_name||'S').slice(0,1)}</div><div><b>{p.seller?.seller_name||'판매자'}</b><span>{level(p.seller)} · 완료 {Number(p.seller?.completed_sales||0).toLocaleString()}건</span></div></div><div className="shopPrice"><strong>{money(p.price)}</strong><button onClick={()=>buy(p)}>구매하기</button></div></div></article>):<div className="marketEmpty">조건에 맞는 상품이 없습니다.</div>}</section>
   </>}

   {view==='sellers'&&<section className="sellerBrowse"><div className="sectionTitle"><h2>직접 거래 가능 판매자</h2><p>실시간 응대가 필요한 거래는 판매자에게 원하는 금액으로 에스크로 요청을 보낼 수 있습니다.</p></div><div className="sellerGrid">{sellers.filter(s=>String(s.user_id)!==String(myId)).length?sellers.filter(s=>String(s.user_id)!==String(myId)).map(s=><article className="sellerCard" key={s.user_id}><div className="sellerCardHead"><div className="sellerBigAvatar">{String(s.seller_name||'S').slice(0,1)}</div><div><h3>{s.seller_name}</h3><span>{level(s)} · 정식 판매자</span></div></div><p>{s.introduction||'판매자 소개가 없습니다.'}</p><div className="sellerMinPrice"><span>판매 가능 최저가</span><strong>{Number(s.minimum_price||0)>0?money(s.minimum_price):'미등록'}</strong></div><div className="sellerStats"><div><b>{Number(s.completed_sales||0).toLocaleString()}</b><span>완료 거래</span></div><div><b>{s.dispute_count||0}</b><span>분쟁</span></div><div><b>{money(s.total_sales_amount)}</b><span>누적 거래</span></div></div><button className="sellerAction" onClick={()=>openDirect(s)}>직접 에스크로 요청</button></article>):<div className="marketEmpty">등록된 판매자가 없습니다.</div>}</div></section>}

   {view==='trades'&&<section className="tradeWorkspace"><div className="sectionTitle"><h2>내 거래</h2><p>상품 구매와 직접 협의 거래의 진행상태를 확인합니다.</p></div><div className="tradeGrid"><div className="tradeList">{trades.length?trades.map(t=><button key={t.id} className={selectedTrade?.id===t.id?'active':''} onClick={()=>openTrade(t)}><b>#{t.id} · {t.title_snapshot}</b><span>{t.trade_type==='DIRECT'?'직접협의':'상품'} · {money(t.amount)} · {t.status}</span></button>):<div className="empty">거래가 없습니다.</div>}</div><div className="tradeDetail">{selectedTrade?<><div className="head"><div><h2>거래 #{selectedTrade.id}</h2><p>{selectedTrade.title_snapshot} · {money(selectedTrade.amount)}</p></div><span className="badge wait">{selectedTrade.status}</span></div><div className="tradeActions">{String(selectedTrade.seller_id)===String(myId)&&['ESCROWED','ACCEPTED'].includes(selectedTrade.status)&&<button className="btn primary" onClick={()=>tradeAction('sellerComplete',selectedTrade.id,'판매완료를 요청할까요?')}>판매완료 요청</button>}{String(selectedTrade.buyer_id)===String(myId)&&['ESCROWED','ACCEPTED','SELLER_COMPLETED'].includes(selectedTrade.status)&&<button className="btn primary" onClick={()=>tradeAction('buyerComplete',selectedTrade.id,`${money(selectedTrade.amount)}를 판매자에게 지급할까요?`)}>거래완료 · 지급</button>}{!['COMPLETED','CANCELLED','ADMIN_RESOLVED'].includes(selectedTrade.status)&&<><button className="btn" onClick={()=>tradeAction('cancelTrade',selectedTrade.id,'거래 취소를 요청할까요?')}>취소 요청</button><button className="btn danger" onClick={()=>tradeAction('disputeTrade',selectedTrade.id,'분쟁을 접수할까요?')}>문제 신고</button></>}</div><div className="tradeChat">{messages.length?messages.map(m=><div className={'tradeMsg '+(m.sender_type==='SYSTEM'?'system':'')} key={m.id}><small>{m.sender_type} · {m.created_at?.replace('T',' ').slice(0,19)}</small><p>{m.message}</p></div>):<div className="empty">대화가 없습니다.</div>}</div><div className="tradeComposer"><input value={chatText} onChange={e=>setChatText(e.target.value)} placeholder="거래 메시지 입력" onKeyDown={e=>e.key==='Enter'&&sendChat()}/><button onClick={sendChat}>전송</button></div></>:<div className="marketEmpty">왼쪽에서 거래를 선택하세요.</div>}</div></div></section>}

   {directOpen&&<div className="modalBack"><div className="modal directModal"><div className="head"><div><h2>직접 에스크로 요청</h2><p>{direct.seller_name} 판매자와 협의할 금액을 지정합니다.</p></div><button className="x" onClick={()=>setDirectOpen(false)}>×</button></div><div className="form"><label className="field"><span>에스크로 금액</span><input className="input" type="number" value={direct.amount} onChange={e=>setDirect({...direct,amount:e.target.value})}/></label><label className="field"><span>카테고리</span><select className="input" value={direct.category_id} onChange={e=>setDirect({...direct,category_id:e.target.value})}><option value="">선택 안 함</option>{categories.map(c=><option value={c.id} key={c.id}>{c.name}</option>)}</select></label><label className="field full"><span>거래 제목</span><input className="input" value={direct.title} onChange={e=>setDirect({...direct,title:e.target.value})}/></label><label className="field full"><span>협의 내용</span><textarea className="textarea" value={direct.description} onChange={e=>setDirect({...direct,description:e.target.value})}/></label></div><div className="escrowNotice">요청 금액은 즉시 에스크로에 보관되며 거래 완료 전까지 판매자에게 지급되지 않습니다.</div><div className="actions"><button className="btn" onClick={()=>setDirectOpen(false)}>취소</button><button className="btn primary" onClick={createDirect}>에스크로 요청</button></div></div></div>}
 </main>;
}
