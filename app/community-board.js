...
export default function CommunityBoard({type}){
 ...
 async function openPost(p){if(p?._notice){setSelected(p);setComments([]);return}try{const r=await api.communityPost(p.id);setSelected(r.item);setComments(r.comments||[])}catch(e){alert(e.message)}}
 ...
 return <main className="communityPage standaloneBoard">
   ...
   <section className="boardWorkspace">
    <div className="postList">{listItems.length?listItems.map(p=><button key={(p._notice?'notice-':'post-')+p.id} className={(selected?.id===p.id&&!!selected?._notice===!!p._notice?'active ':'')+(p.is_pinned?'pinned ':'')+(p._notice?'pinned':'')} onClick={()=>openPost(p)}><div><b>{p._notice&&<span className="pinBadge">공지</span>}{!p._notice&&p.is_pinned&&<span className="pinBadge">고정</span>}{p.title}</b><span>{p.author_name||'회원'} · {date(p.created_at)}</span></div><div>{p._notice?<span>필독</span>:<><span>댓글 {p.comment_count||0}</span><span>조회 {p.view_count||0}</span></>}</div></button>):<div className="boardEmpty">등록된 글이 없습니다.</div>}</div>
    <div className="postDetail">{selected?<><div className="postTitle"><div className="postTitleTop"><h2>{selected._notice&&<span className="pinBadge">공지</span>}{!selected._notice&&selected.is_pinned&&<span className="pinBadge">고정</span>}{selected.title}</h2>{!selected._notice&&permissions.board_admin&&<button className="pinAction" onClick={togglePin}>{selected.is_pinned?'고정해제':'고정하기'}</button>}</div><div>{selected._notice?'ANGEL PAY 운영팀':(selected.author_name||'회원')} · {date(selected.created_at)}{!selected._notice&&` · 조회 ${selected.view_count||0}`}</div></div><div className="postBody">{selected.content}</div>{!selected._notice&&<div className="commentSection"><h3>댓글 {comments.length}</h3><div className="commentList">{comments.length?comments.map(c=><div className="commentItem" key={c.id}><div><b>{c.author_name||'회원'}</b><span>{date(c.created_at)}</span></div><p>{c.content}</p></div>):<div className="commentEmpty">첫 댓글을 남겨보세요.</div>}</div><div className="commentComposer"><textarea value={comment} onChange={e=>setComment(e.target.value)} placeholder="댓글을 입력하세요."/><button disabled={busy} onClick={submitComment}>댓글 등록</button></div></div>}</>:<div className="boardEmpty large">왼쪽에서 글을 선택하세요.</div>}</div>
   </section>
