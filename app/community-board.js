...
export default function CommunityBoard({type}){
 ...
 async function togglePin(){if(!selected||!permissions.board_admin)return;const next=!selected.is_pinned;try{await api.pinCommunityPost(selected.id,next);await loadPostsOnly();setSelected({...selected,is_pinned:next});alert(next?'게시글을 상단에 고정했습니다.':'게시글 고정을 해제했습니다.')}catch(e){alert(friendly(e))}}
 ...
 return <main className="communityPage standaloneBoard">
   ...
   <section className="boardWorkspace">
    <div className="postList">{posts.length?posts.map(p=><button key={p.id} className={(selected?.id===p.id?'active ':'')+(p.is_pinned?'pinned':'')} onClick={()=>openPost(p)}><div><b>{p.is_pinned&&<span className="pinBadge">고정</span>}{p.title}</b><span>{p.author_name||'회원'} · {date(p.created_at)}</span></div><div><span>댓글 {p.comment_count||0}</span><span>조회 {p.view_count||0}</span></div></button>):<div className="boardEmpty">등록된 글이 없습니다.</div>}</div>
    <div className="postDetail">{selected?<><div className="postTitle"><div className="postTitleTop"><h2>{selected.is_pinned&&<span className="pinBadge">고정</span>}{selected.title}</h2>{permissions.board_admin&&<button className="pinAction" onClick={togglePin}>{selected.is_pinned?'고정해제':'고정하기'}</button>}</div><div>{selected.author_name||'회원'} · {date(selected.created_at)} · 조회 {selected.view_count||0}</div></div><div className="postBody">{selected.content}</div><div className="commentSection"><h3>댓글 {comments.length}</h3><div className="commentList">{comments.length?comments.map(c=><div className="commentItem" key={c.id}><div><b>{c.author_name||'회원'}</b><span>{date(c.created_at)}</span></div><p>{c.content}</p></div>):<div className="commentEmpty">첫 댓글을 남겨보세요.</div>}</div><div className="commentComposer"><textarea value={comment} onChange={e=>setComment(e.target.value)} placeholder="댓글을 입력하세요."/><button disabled={busy} onClick={submitComment}>댓글 등록</button></div></div></>:<div className="boardEmpty large">왼쪽에서 글을 선택하세요.</div>}</div>
   </section>
