...
export default function AdminReferralSettings(){
 ...
 return <main className="content" style={{maxWidth:900,margin:'0 auto'}}><div className="head"><div><h1>추천인 설정</h1><p>추천왕 유효회원 기준과 반복 자금순환 보상 상한을 관리합니다.</p></div></div><section className="card"><div className="form"><label className="field"><span>유효추천 인정 최소 첫 승인 충전</span><input className="input" type="number" min="0" value={qualification} onChange={e=>setQualification(e.target.value)}/></label><label className="field"><span>추천회원 1명당 월 보상 상한</span><input className="input" type="number" min="0" value={cap} onChange={e=>setCap(e.target.value)}/></label></div><div className="note" style={{marginTop:12}}>현재 기준: 첫 승인 충전 {fmt(qualification)} 이상이면 추천왕 유효회원으로 인정됩니다. 해당 추천회원으로부터 발생하는 1% 추천보상은 월 {fmt(cap)}까지 지급됩니다. 0P로 설정하면 해당 상한을 사용하지 않습니다.</div><div className="actions"><button className="btn primary" disabled={busy} onClick={save}>{busy?'저장 중...':'설정 저장'}</button></div></section></main>;
}
