...
export const api={
  ...
  sellerMe:()=>request('/v1/market/seller/me'),sellerApply:(payload)=>request('/v1/market/seller/apply',{method:'POST',body:JSON.stringify(payload)}),
  marketProducts:(categoryId='')=>request(`/v1/market/products${categoryId?`?category_id=${categoryId}`:''}`),
  createProduct:(payload)=>request('/v1/market/products',{method:'POST',body:JSON.stringify(payload)}),buyProduct:(id,quantity=1)=>request(`/v1/market/products/${id}/buy`,{method:'POST',body:JSON.stringify({quantity})}),
